...
function initThreeJS() {
  ...
  // Lighting — warm cardiac feel
  const ambient = new THREE.AmbientLight(0x1a0505, 1.5);
  scene.add(ambient);

  const mainLight = new THREE.DirectionalLight(0xff6655, 2.5);
  ...
  const fillLight = new THREE.DirectionalLight(0x2244aa, 1.2);
  ...
  const topLight = new THREE.PointLight(0xff8866, 1.5, 8);
...
function buildPericardium() {
  // Thin outer sac — very subtle, slightly transparent
  const geo = new THREE.SphereGeometry(1.65, 32, 32);
  // Deform to heart-like shape
  const pos = geo.attributes.position;
  for (let i = 0; i < pos.count; i++) {
    const x = pos.getX(i), y = pos.getY(i), z = pos.getZ(i);
    const r = Math.sqrt(x * x + y * y + z * z);
    const theta = Math.acos(y / r);
    const scale = 1 + 0.06 * Math.sin(2 * theta);
    pos.setXYZ(i, x * scale, y * (0.92 - 0.12 * (y / r)), z * scale * 0.85);
  }
  geo.computeVertexNormals();
  const mat = new THREE.MeshStandardMaterial({
    color: 0xcc4444, transparent: true, opacity: 0.06,
...
function buildLeftVentricle() {
  // Largest, most muscular — bottom-left, dominant
  const geo = new THREE.SphereGeometry(0.82, 32, 32);
  const pos = geo.attributes.position;
  for (let i = 0; i < pos.count; i++) {
    let x = pos.getX(i), y = pos.getY(i), z = pos.getZ(i);
    // Elongate downward into cone-like ventricle shape
    if (y < 0) y *= 1.6;
    x *= 0.8; z *= 0.75;
    // Thicken the wall — slight bulge
  ...
  // Add muscular ridges (trabeculae)
  for (let i = 0; i < 5; i++) {
    const ridge = createRidge(0x9b2222);
    ridge.position.set(-0.18 + (Math.random() - 0.5) * 0.3, -0.6 + i * 0.15, 0.5 + (Math.random() - 0.5) * 0.2);
    ridge.rotation.z = Math.PI / 2 + (Math.random() - 0.5) * 0.5;
    ridge.scale.set(0.4, 0.08, 0.08);
    heartGroup.add(ridge);
  }
}

function buildRightVentricle() {
  const geo = new THREE.SphereGeometry(0.68, 28, 28);
  const pos = geo.attributes.position;
  for (let i = 0; i < pos.count; i++) {
    let x = pos.getX(i), y = pos.getY(i), z = pos.getZ(i);
    if (y < 0) y *= 1.4;
...
function buildLeftAtrium() {
  const geo = new THREE.SphereGeometry(0.62, 28, 28);
  const pos = geo.attributes.position;
  for (let i = 0; i < pos.count; i++) {
    let x = pos.getX(i), y = pos.getY(i), z = pos.getZ(i);
    y *= 0.7; x *= 0.85; z *= 0.8;
  ...
  // Left auricle (ear-like appendage)
  const aurGeo = new THREE.SphereGeometry(0.22, 16, 16);
  const aurMat = heartMat(0xd44040, 1, 0x6a1a1a, 0.6);
  const aur = new THREE.Mesh(aurGeo, aurMat);
  aur.position.set(-0.75, 0.55, 0.2);
  aur.scale.set(1.2, 0.7, 0.6);
  heartGroup.add(aur);
}

function buildRightAtrium() {
  const geo = new THREE.SphereGeometry(0.58, 28, 28);
  const pos = geo.attributes.position;
  for (let i = 0; i < pos.count; i++) {
    let x = pos.getX(i), y = pos.getY(i), z = pos.getZ(i);
  ...
  // Right auricle
  const aurGeo = new THREE.SphereGeometry(0.2, 16, 16);
  const aurMat = heartMat(0x5ba0d0, 1, 0x1a3a5a, 0.6);
  const aur = new THREE.Mesh(aurGeo, aurMat);
  aur.position.set(0.9, 0.6, 0.15);
  aur.scale.set(1.1, 0.65, 0.55);
  heartGroup.add(aur);
}
...
function buildSurfaceTexture() {
  // Add surface vessels / texture bumps to make heart look real
  const positions = [
    [-0.1, 0.3, 0.7], [0.2, 0.1, 0.72], [-0.3, -0.1, 0.68],
    [0.1, -0.3, 0.7], [-0.15, -0.5, 0.65], [0.3, 0.4, 0.6],
  ];
  positions.forEach(([x, y, z]) => {
    const geo = new THREE.SphereGeometry(0.025 + Math.random() * 0.02, 6, 6);
    const mat = heartMat(0xaa2222, 1, 0x550000, 0.7, 0.05);
...
function createRidge(color) {
  const geo = new THREE.SphereGeometry(1, 6, 6);
  const mat = heartMat(color, 1, 0x330000, 0.7, 0.05);
...
function highlightPart(partId) {
  // Flash the part briefly
  const partMap = {
    'left_ventricle': heartObjects.leftVentricle,
    'right_ventricle': heartObjects.rightVentricle,
    'left_atrium': heartObjects.leftAtrium,
    'right_atrium': heartObjects.rightAtrium,
    'aorta': heartObjects.aorta,
  };
  const obj = partMap[partId];
  if (obj && obj.material) {
    const origEmissive = obj.material.emissive.getHex();
